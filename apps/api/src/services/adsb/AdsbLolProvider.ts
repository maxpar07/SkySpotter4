// apps/api/src/services/adsb/AdsbLolProvider.ts
//
// Free, keyless ADS-B provider (community-run, readsb-based backend —
// same response shape family as ADS-B Exchange and adsb.fi). This is the
...
export class AdsbLolProvider implements ADSBProvider {
  ...
  async fetchNearby(query: ADSBQuery): Promise<RawAircraft[]> {
    ...
    try {
      ...
      throw new Error("ADSB.lol returned an invalid (non-JSON) response");
    }
